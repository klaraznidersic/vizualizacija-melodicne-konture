...
def extract_metadata(elements, stream):
    ...
    type = classify_adams_contour(first_note, last_note, max_pitch, min_pitch, position_max)
    metadata['Adams'] = type
...
def contour(element_list):
    ...
    return df, reduced_df, interval_df, polynom_df, heatmap_df, gauss_df # po potrebi: df <-> note_starts
...
def main_page():
    ...
    def create_interactive_plot(song):
        ...
        if song.get('show_toplotni'):
            # 1. Prepare Data
        # Ensure they are numpy arrays to prevent indexing errors
            x_val = np.array(song['heatmap']['time'])
            y_val = np.array(song['heatmap']['pitch'])
            
            # Safety check: KDE requires at least 2 distinct points
            if len(x_val) > 1:
                # 2. Create the Grid (The "Canvas")
                # Define the range with a small buffer
                ...
                # Flatten the grid to pass into the kernel
                ...
                # --- BANDWIDTH CONTROL ---
                # 0.5 makes it "tighter" (more detailed), matching your previous request
                kernel.set_bandwidth(bw_method=kernel.factor * 0.5)
                
                # Calculate Z (density) and reshape back to grid shape
                zi = np.reshape(kernel(positions).T, xi_grid.shape)
                
                # 4. Add the Trace
                # Note: We pass the 1D arrays (xi, yi) for axes, and 2D array (zi) for data.
                # This ensures Plotly maps Time to X and Pitch to Y correctly.
                fig.add_trace(go.Contour(
                    x=xi,
                    y=yi,
                    z=zi,
                    colorscale='GnBu',
                    opacity=0.6,          # Semi-transparent so grid lines show through
                    showscale=False,      # Hide the color bar sidebar
                    
                    # --- THRESHOLD & STYLE ---
                    contours=dict(
                        # Start slightly above 0 to create the "threshold" effect (remove background noise)
                        start=zi.max() * 0.05, 
                        end=zi.max(),
                        size=zi.max() * 0.05, # Resolution of the rings
                        showlines=False,      # smooth look
                        coloring='fill'
                    ),
                    hoverinfo='skip', name='Heatmap'), row=1, col=1)

            # fig.add_trace(go.Histogram2dContour(x=song['heatmap']['time'],y=song['heatmap']['pitch'],
            #                                     colorscale='GnBu',showscale=False, opacity=0.6, ncontours=30,
            #                                     contours=dict(showlines=False, coloring='heatmap'), hoverinfo='skip'), row=1, col=1)
        if show_intervals:
            ...
            fig.update_yaxes(title_text="Interval", range=[-9, 9], tickvals=[-8, -6, -4, -2, 0, 2, 4, 6, 8], row=2, col=1)
    ...
    def render_song_card(song):
        with ui.card().classes('w-full p-2'):
            ...
            with container:                                
                ...
                with ui.dialog() as large_dialog, ui.card().classes('w-full max-w-6xl h-[90vh] p-0'):                    
